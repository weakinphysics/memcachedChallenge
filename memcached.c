...
/*
    I 733L 17 D33P W17H1N, 17S JUS7 B3N3A7H 7H3 SK1N

    MODIFYING MEMCACHED TO SUPPORT COMMAND: MULT
    Usage: mult takes two operands; the first operand is the field name, the second is 32 bit integer. 
    Example: mult weight 24
    The second operand may be any integer such that the result is a 32 bit number, since we do not yet know whether memcached supports bigger integers

*/

static void process_mulitply_command(conn *c, token_t *tokens, const size_t ntokens, const size_t operand){
    uint64_t multiplier;
    char* key;
    size_t nkey;
    char temp[INCR_MAX_STORAGE_LEN];
    ...
    if(tokens[KEY_TOKEN].length > KEY_MAX_LENGTH){
        out_string(c, "CLIENT_ERROR, bad command line format: TOKEN LENGTH GREATER THAN MAX TOKEN LENGTH");
        return;
    }
    ...
    if(!safe_strtoull(tokens[2].value, &multiplier)){
        out_string(c, "CLIENT ERROR, invalid numeric multiplier argument");
        return;
    }

    switch(mult_number(c, key, nkey, multiplier, temp, NULL)){
        case OK:
            out_string(c, temp);
            break;
        case NON_NUMERIC: 
            out_string(c, "CLIENT ERROR, target data is non numeric");
            break;
        case EOM:
        ...
        case DELTA_ITEM_NOT_FOUND:
            ...
            break;
        case  DELTA_ITEM_CAS_MISMATCH:
            out_string(c, "THIS SHOULD NOT HAPPEN! RUN FOR YOUR LIVES!");
            break;
    }
}
...
enum delta_result_type multiply_by_integer(conn *c, const char *key, const size_t nkey,
    ...
    if (!safe_strtoull(ptr, &curr_val)) {
        do_item_remove(it);
        return NON_NUMERIC;
    }
    curr_val *= multiplier;
    MEMCACHED_COMMAND_INCR(c->sfd, ITEM_key(it), it->nkey, curr_val);


    snprintf(buf, INCR_MAX_STORAGE_LEN, "%llu", (unsigned long long)curr_val);
    res = strlen(buf);
    if (res + 2 > it->nbytes || it->refcount != 1) { /* need to realloc */
        item *new_it;
        new_it = do_item_alloc(ITEM_key(it), it->nkey, atoi(ITEM_suffix(it) + 1), it->exptime, res + 2, hv);
...
static void process_command(conn *c, char *command) {
    ...
    if (ntokens >= 3 &&
        ...
        printf("Your mom gay\n");
        process_mulitply_command(c, tokens, ntokens, 1);
